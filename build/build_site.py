...
# Keyed by (target_dir, run_tag, normalized job name) — the granularity at which a fold
# is actually unique. A bare job name is NOT unique: the same legacy candidate id occurs
# under several run_tags and several targets, so a name-keyed index silently overwrote
# folds and could attach the wrong structure. Identity comes from the path plus the
# candidate record, never parsed back out of a name.
idx = {}
for root, dirs, files in os.walk('resultv3/hotspot'):
    ...
    for fn in files:
        ...
        cif = os.path.join(root, fn).replace(os.sep, '/')
        parts = os.path.dirname(cif).split('/')      # .../hotspot/<cat>/<tgt>/af3/<tag>/<job>
        if len(parts) < 6 or parts[-3] != 'af3': continue
        idx[('/'.join(parts[-6:-3]), parts[-2], norm(base))] = cif
print('indexed model_0.cif files:', len(idx))

def cif_key(c):
    """Locate a candidate's fold by its own recorded identity, never by name alone."""
    return (c['target_dir'], c['run_tag'], norm(c['legacy_id']))
...
hits = [c for c in scored if c.get('triage') == 'ACCEPT' and cif_key(c) in idx]
hits.sort(key=lambda c: -comp(c))
# `uid` is unique by construction (batch_v3.py asserts it), so there is nothing to dedup:
# two rows sharing a legacy id are DIFFERENT designs with different sequences.
dup = [u for u, n in collections.Counter(c['uid'] for c in hits).items() if n > 1]
if dup:
    raise SystemExit('duplicate uid among ACCEPT hits (should be impossible): %s' % dup[:5])
grank = {c['uid']: i + 1 for i, c in enumerate(hits)}
n_missing = sum(1 for c in scored if c.get('triage') == 'ACCEPT' and cif_key(c) not in idx)
print('ACCEPT (position-verified, composite >=', THR, '):', len(hits),
      'designs (', n_missing, 'with no fold on disk )')

# ---- per-design: CIF text, chain roles, PAE png, pLDDT ----
CIFS = {}; binder_objs = {}
for k, c in enumerate(hits):
    cid = c['uid']; cif = idx[cif_key(c)]; jf = os.path.dirname(cif); fid = os.path.basename(jf)
    ...
    a = c['af3']
    binder_objs[cid] = {'id': cid, 'legacy_id': c['legacy_id'], 'run_tag': c['run_tag'],
        'grank': grank[cid], 'peptide': c['peptide'], 'mhc': c['mhc'],
...
for (pep, mhc), cs in groups.items():
    cs = sorted(cs, key=lambda c: -comp(c)); bl = [binder_objs[c['uid']] for c in cs]
